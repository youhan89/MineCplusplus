...
object Sfx {
    ...
    fun blockToSequential(location: Location, plugin: CubematicPlugin) {
        ...
        location.world.playSound(location, sound, 1f, 0.15f)
        (1..8).forEach {
            plugin.scheduleRun(it.toLong() * 3) {
                location.world.playSound(location, sound, 1f, 0.1f * it + 0.15f)
            }
        }
    }
    fun blockToSequential2(location: Location, plugin: CubematicPlugin) {
        ...
        location.world.playSound(location, sound, 1f, 0.5f)

        plugin.scheduleRun(9) {
            location.world.playSound(location, sound, 1f, 0.75f)
        }
